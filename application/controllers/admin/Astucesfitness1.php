...
class Astucesfitness extends MY_Controller {
	public function __construct(){
		...
		public function datatable_json(){				   					   
			...
			{  
				...
				$data[]= array(
					...
					'<a title="View" class="view btn btn-sm btn-info" href="'.base_url('admin/astucesfitness/edit/'.$row['id']).'"> <i class="material-icons">visibility</i></a>
					<a title="Edit" class="update btn btn-sm btn-primary" href="'.base_url('admin/astucesfitness/edit/'.$row['id']).'"> <i class="material-icons">edit</i></a>
					<a title="Delete" class="delete btn btn-sm btn-danger '.$disabled.'" data-href="'.base_url('admin/astucesfitness/del/'.$row['id']).'" data-toggle="modal" data-target="#confirm-delete"> <i class="material-icons">delete</i></a>
					',
		...
		//-----------------------------------------------------------------------
		public function add(){
			$data['user_groups'] = $this->user_model->get_alertes_groups();
			if($this->input->post('submit')){
				$this->form_validation->set_rules('dateheureenvoi', 'Firstname', 'trim|required');
				$this->form_validation->set_rules('message', 'Lastname', 'trim|required');
				if ($this->form_validation->run() == FALSE) {
					...
					$data = array(
						'dateheureenvoi' => $this->input->post('username'),
					...
					$result = $this->alertes_model->alertes_user($data);
					if($result){
						// Add User Activity
