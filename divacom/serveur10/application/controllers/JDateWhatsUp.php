...
class JDATEWHATSUP extends CI_Controller
 {

		public function __construct(){

			parent::__construct();
			$this->load->model('jdwu_model');
		}
		...
		//$numero=substr($msisdn, 3);
			...
			$this->load->view('view_accueilJDWU',$data);
		...
		{
			$this->jdwu_model->get_offre();

			$data['offre_montant']=$this->jdwu_model->offre_montant();
			$data['offre_m']=$this->jdwu_model->offre_montant();
 ...
 */
			if(isset($_SERVER['HTTP_USER_OFFRE'])){
 			...
 			if (isset($offre)) {
	   			...
	   			$data['offre']= $this->jdwu_model->get_offre();
...
*/
				$data= $this->jdwu_model->souscription_user($numero);
 //echo $numero;
		//var_dump($data);
					...
					*/    		$dateJourCompare = date("Y-m-d");
								...
								$idsous=$row['idsous'];
											if ($dateJourCompare > $row['datefinsous']) {
													$update_souscription_reabon=$this->jdwu_model->update_souscription($dateJour,$dateFin,$idoffre,$token,$idsous);
							...
							//	$test="[".$dateJour."] Reabonnement :".$numero."".PHP_EOL;
								//fputs($files,$test);				
											...
											$update_souscription_prolabon=$this->jdwu_model->update_souscription_prolabon($dateJour,$dateF,$idoffre,$token,$idsous);
				...
				$param['idsous']=$idsous;
				$insert_stat=$this->jdwu_model->insert_stat_souscription($param);
			//$test="[".$dateJour."] Reabonnement :".$numero."".PHP_EOL;
			//fputs($files,$test);
				$sms=1; 
           ...
           // $param['idsous']=$idsous;
			...
			$insert_souscript=$this->jdwu_model->insert_souscription($param_sous);
			...
			//mysql_query($sqlstat); 
			$insert_stat=$this->jdwu_model->insert_stat_souscription($param_stat);
	 ...
	 //fclose($files);
		...
		if($sms==1){
			...
			$message=utf8_encode("Votre abonnement au service Agenda Event a été prolongé".$duree." jr".$montantoffre."F. Pour vous débonner, composez #303*2389#");
	   ...
	   /*  if(isset($_POST['option'])){
		...
		public function desabonner_confirm()
		{
			...
			$dateFin = date('Y-m-d H:i:s', $date);
			   ...
			   $search=$this->jdwu_model->search_souscription_user($numero);
		   
			   if(!empty($search)){
				$data['souscription']= $this->jdwu_model->souscription_user($numero);

				$idsous=$data['souscription'][0]['idsous'];
				   //Modification de l'etat dans la table souscription
				   $update=$this->jdwu_model->update_souscription_desabon($idsous);
				   ...
				   $insert_desous=$this->jdwu_model->insert_desouscription($param_desous);
