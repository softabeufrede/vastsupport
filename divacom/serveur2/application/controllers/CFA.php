...
class Cfa extends CI_Controller
 {
		...
		//$numero=substr($msisdn, 3);
			...
			$this->load->view('view_accueilcfa',$data);
 ...
 */
			if(isset($_SERVER['HTTP_USER_OFFRE'])){
 			...
 			if (isset($offre)) {
	   			...
	   			$data['offre']= $this->Cfa_model->get_offre();
...
*/
				$data= $this->Cfa_model->souscription_user($numero);
 //echo $numero;
		//var_dump($data);
					...
					*/    		$dateJourCompare = date("Y-m-d");
								...
								$idsous=$row['idsous'];
											if ($dateJourCompare > $row['datefinsous']) {
													$update_souscription_reabon=$this->Cfa_model->update_souscription($dateJour,$dateFin,$idoffre,$token,$idsous);
							...
							//	$test="[".$dateJour."] Reabonnement :".$numero."".PHP_EOL;
								//fputs($files,$test);				
											...
											$update_souscription_prolabon=$this->Cfa_model->update_souscription_prolabon($dateJour,$dateF,$idoffre,$token,$idsous);
				...
				$param['idsous']=$idsous;
				$insert_stat=$this->Cfa_model->insert_stat_souscription($param);
			//$test="[".$dateJour."] Reabonnement :".$numero."".PHP_EOL;
			//fputs($files,$test);
				$sms=1; 
           ...
           // $param['idsous']=$idsous;
			...
			$insert_souscript=$this->Cfa_model->insert_souscription($param_sous);
			...
			//mysql_query($sqlstat); 
			$insert_stat=$this->Cfa_model->insert_stat_souscription($param_stat);
	 ...
	 //fclose($files);
		...
		if($sms==1){
			...
			$message=utf8_encode("Votre abonnement au service Conseil finance et Argent a été prolongé".$duree." jr".$montantoffre."F. Pour vous débonner, composez #303*2389#");
	
		}else{
		 $message=utf8_encode("Vous êtes abonné au service Conseil finance et Argent à".$montantoffre." FCFA/".$liboffre.". Pour vous débonner, composez #303*2389#");
	   ...
	   /*  if(isset($_POST['option'])){
		...
		public function desabonner_confirm()
		{
			...
			$dateFin = date('Y-m-d H:i:s', $date);
			   ...
			   $search=$this->Cfa_model->search_souscription_user($numero);
		   
			   if(!empty($search)){
				$data['souscription']= $this->Cfa_model->souscription_user($numero);

				$idsous=$data['souscription'][0]['idsous'];
				   //Modification de l'etat dans la table souscription
				   $update=$this->Cfa_model->update_souscription_desabon($idsous);
				   ...
				   $insert_desous=$this->Cfa_model->insert_desouscription($param_desous);
