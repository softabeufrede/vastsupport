...
class JobBooster extends CI_Controller
 {
		...
		//$numero=substr($msisdn, 3);
			...
			$this->load->view('view_accueilJB',$data);
		...
		{
			$this->jb_model->get_offre();

			$data['offre_montant']=$this->Jb_model->offre_montant();
			$data['offre_m']=$this->Jb_model->offre_montant();
 ...
 */
			if(isset($_SERVER['HTTP_USER_OFFRE'])){
 			...
 			if (isset($offre)) {
	   			...
	   			$data['offre']= $this->Jb_model->get_offre();
...
*/
				$data= $this->Jb_model->souscription_user($numero);
 //echo $numero;
		//var_dump($data);
					...
					*/    		$dateJourCompare = date("Y-m-d");
								...
								$idsous=$row['idsous'];
											if ($dateJourCompare > $row['datefinsous']) {
													$update_souscription_reabon=$this->Jb_model->update_souscription($dateJour,$dateFin,$idoffre,$token,$idsous);
							...
							//	$test="[".$dateJour."] Reabonnement :".$numero."".PHP_EOL;
								//fputs($files,$test);				
											...
											$update_souscription_prolabon=$this->Jb_model->update_souscription_prolabon($dateJour,$dateF,$idoffre,$token,$idsous);
				...
				$param['idsous']=$idsous;
				$insert_stat=$this->Jb_model->insert_stat_souscription($param);
			//$test="[".$dateJour."] Reabonnement :".$numero."".PHP_EOL;
			//fputs($files,$test);
				$sms=1; 
           ...
           // $param['idsous']=$idsous;
			...
			$insert_souscript=$this->Jb_model->insert_souscription($param_sous);
			...
			//mysql_query($sqlstat); 
			$insert_stat=$this->Jb_model->insert_stat_souscription($param_stat);
	   ...
	   /*  if(isset($_POST['option'])){
		...
		public function desabonner_confirm()
		{
			...
			$dateFin = date('Y-m-d H:i:s', $date);
			   ...
			   $search=$this->Jb_model->search_souscription_user($numero);
		   
			   if(!empty($search)){
				$data['souscription']= $this->Jb_model->souscription_user($numero);

				$idsous=$data['souscription'][0]['idsous'];
				   //Modification de l'etat dans la table souscription
				   $update=$this->Jb_model->update_souscription_desabon($idsous);
				   ...
				   $insert_desous=$this->Jb_model->insert_desouscription($param_desous);
