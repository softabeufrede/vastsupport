...
class Sos extends CI_Controller
 {
		...
		//$numero=substr($msisdn, 3);
			...
			$this->load->view('view_accueilSOS',$data);
		...
		{
			$this->Sos_model->get_offre();

			$data['offre_montant']=$this->Sos_model->offre_montant();
			$data['offre_m']=$this->Sos_model->offre_montant();
 ...
 */
			if(isset($_SERVER['HTTP_USER_OFFRE'])){
 			...
 			if (isset($offre)) {
	   			...
	   			$data['offre']= $this->Sos_model->get_offre();
...
*/
				$data= $this->Sos_model->souscription_user($numero);
 //echo $numero;
		//var_dump($data);
					...
					*/    		$dateJourCompare = date("Y-m-d");
								...
								$idsous=$row['idsous'];
											if ($dateJourCompare > $row['datefinsous']) {
													$update_souscription_reabon=$this->Sos_model->update_souscription($dateJour,$dateFin,$idoffre,$token,$idsous);
							...
							//	$test="[".$dateJour."] Reabonnement :".$numero."".PHP_EOL;
								//fputs($files,$test);				
											...
											$update_souscription_prolabon=$this->Sos_model->update_souscription_prolabon($dateJour,$dateF,$idoffre,$token,$idsous);
				...
				$param['idsous']=$idsous;
				$insert_stat=$this->Sos_model->insert_stat_souscription($param);
			//$test="[".$dateJour."] Reabonnement :".$numero."".PHP_EOL;
			//fputs($files,$test);
				$sms=1; 
           ...
           // $param['idsous']=$idsous;
			...
			$insert_souscript=$this->Sos_model->insert_souscription($param_sous);
			...
			//mysql_query($sqlstat); 
			$insert_stat=$this->Sos_model->insert_stat_souscription($param_stat);
	   ...
	   /*  if(isset($_POST['option'])){
		...
		public function desabonner_confirm()
		{
			...
			$dateFin = date('Y-m-d H:i:s', $date);
			   ...
			   $search=$this->Sos_model->search_souscription_user($numero);
		   
			   if(!empty($search)){
				$data['souscription']= $this->Sos_model->souscription_user($numero);

				$idsous=$data['souscription'][0]['idsous'];
				   //Modification de l'etat dans la table souscription
				   $update=$this->Sos_model->update_souscription_desabon($idsous);
				   ...
				   $insert_desous=$this->Sos_model->insert_desouscription($param_desous);
