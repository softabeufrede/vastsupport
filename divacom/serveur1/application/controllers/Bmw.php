...
class Bmw extends CI_Controller
 {

		public function __construct(){

			parent::__construct();
			$this->load->model('Bmw_model');
		}
		...
		//$numero=substr($msisdn, 3);
			...
			$this->load->view('View_AccueilBmw',$data);
		...
		{
			$this->Bmw_model->get_offre();

			$data['offre_montant']=$this->Bmw_model->offre_montant();
			$data['offre_m']=$this->Bmw_model->offre_montant();
...
*/
				$data= $this->Bmw_model->souscription_user($numero);
 //echo $numero;
		//var_dump($data);
					...
					*/    		$dateJourCompare = date("Y-m-d");
								...
								$idsous=$row['idsous'];
											if ($dateJourCompare > $row['datefinsous']) {
													$update_souscription_reabon=$this->Bmw_model->update_souscription($dateJour,$dateFin,$idoffre,$token,$idsous);
							...
							//	$test="[".$dateJour."] Reabonnement :".$numero."".PHP_EOL;
								//fputs($files,$test);				
											...
											$update_souscription_prolabon=$this->Bmw_model->update_souscription_prolabon($dateJour,$dateF,$idoffre,$token,$idsous);
				...
				$param['idsous']=$idsous;
				$insert_stat=$this->Bmw_model->insert_stat_souscription($param);
			//$test="[".$dateJour."] Reabonnement :".$numero."".PHP_EOL;
			//fputs($files,$test);
				$sms=1; 
           ...
           // $param['idsous']=$idsous;
			...
			$insert_souscript=$this->MotivPlus_model->insert_souscription($param_sous);
			...
			//mysql_query($sqlstat); 
			$insert_stat=$this->MotivPlus_model->insert_stat_souscription($param_stat);
	 ...
	 //fclose($files);
		...
		if($sms==1){
			...
			$message=utf8_encode("Votre abonnement au service Motiv Plus a été prolongé".$duree." jr".$montantoffre."F. Pour vous débonner, composez #303*2389#");
	
		}else{
		 $message=utf8_encode("Vous êtes abonné au service Motiv Plus à".$montantoffre." FCFA/".$liboffre.". Pour vous débonner, composez #303*2389#");
		...
		public function desabonner_confirm()
		{
			...
			if(isset($_SERVER['HTTP_USER_OFFRE'])){
 			...
 			if (isset($offre)) {
	   			...
	   			$data['offre']= $this->MotivPlus_model->get_offre();
			...
			$dateFin = date('Y-m-d H:i:s', $date);
			   ...
			   $search=$this->MotivPlus_model->search_souscription_user($numero);
		   
			   if(!empty($search)){
				$data['souscription']= $this->MotivPlus_model->souscription_user($numero);

				$idsous=$data['souscription'][0]['idsous'];
				   //Modification de l'etat dans la table souscription
				   $update=$this->motivplus_model->update_souscription_desabon($idsous);
				   ...
				   $insert_desous=$this->MotivPlus_model->insert_desouscription($param_desous);
			...
			//	   fputs($files,$test);
			  // fclose($files);
			   ...
			   $message=utf8_encode("Votre désabonnement au service Motiv Plus a été effectué avec succes. Pour vous réabonner, composez #303*2389#");
